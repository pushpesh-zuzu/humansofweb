const NAV_ITEMS = [
  // { label: "Home", href: "/" },
  {
    label: "Industry Expertise",
    href: "/industry-expertise",
    children: [
      {
        label: "Ecommerce",
        href: "/ecommerce-marketing-agency",
        subItems: [
          {
            label: "E-Commerce SEO",
            href: "/ecommerce-seo-services",
          },
          {
            label: "E-Commerce Website Development",
            href: "/ecommerce-website-design",
          },
          {
            label: "E-Commerce PPC",
            href: "/ecommerce-ppc-services",
          },
          {
            label: "E-Commerce GEO Services",
            href: "/ecommerce-geo-services",
          },
          {
            label: "E-Commerce Social Media Marketing",
            href: "/ecommerce-social-media-marketing",
          },
          { label: "E-Commerce CRO", href: "/ecommerce-cro" },
          {
            label: "E-Commerce Email Marketing",
            href: "/ecommerce-email-marketing",
          },
          {
            label: "E-Commerce Content Marketing",
            href: "/ecommerce-content-marketing",
          },
          {
            label: "Amazon Marketing",
            href: "/amazon-marketing",
          },
        ],
      },

      {
        label: "Dental",
        href: "/dental-digital-marketing-agency",
        subItems: [
          { label: "Dental SEO", href: "/dental-seo-services" },
          { label: "Dental PPC", href: "/dental-ppc-services" },
          { label: "Dental Website Development", href: "/dental-website-design-development-services" },
          { label: "Dental Social Media Marketing", href: "/dental-social-media-marketing-services" },
          { label: "Dental Reputation Management", href: "/dental-reputation-management-services" },
        ],
      },
      {
        label: "Healthcare",
        href: "/healthcare-marketing-agency",
        subItems: [
          { label: "Healthcare SEO", href: "/healthcare-seo-services" },
          { label: "Healthcare PPC", href: "/healthcare-ppc-services" },
          { label: "Healthcare Website Design & Development", href: "/healthcare-website-design-development-services" },
          { label: "Healthcare Link Building", href: "/healthcare-link-building-services" },
          { label: "Healthcare Social Media Marketing", href: "/healthcare-social-media-marketing-services" },
        ],
      },
      {
        label: "Ophthalmology",
        href: "/ophthalmology-digital-marketing-agency",
        subItems: [
          { label: "Ophthalmology SEO", href: "/ophthalmology-seo-services" },
          { label: "Ophthalmology PPC", href: "/ophthalmology-ppc-services" },
          { label: "Ophthalmology Website Design & Development", href: "/ophthalmology-website-design-services" },
        ],
      },
      {
        label: "Chiropractor",
        href: "/chiropractor-digital-marketing-agency",
        subItems: [
          { label: "Chiropractor SEO", href: "/chiropractor-seo-services" },
          { label: "Chiropractor PPC", href: "/chiropractor-ppc-services" },
          { label: "Chiropractor Website Design & Development", href: "/chiropractor-website-design-development-services" },
          { label: "Chiropractor Social Media Marketing", href: "/chiropractor-social-media-marketing-services" },
          { label: "Chiropractor Reputation Management", href: "/chiropractor-reputation-management-services" },
        ],
      },
      {
        label: "Plastic Surgeon",
        href: "/plastic-surgeon-digital-marketing-agency",
        subItems: [
          { label: "Plastic Surgeon SEO", href: "/plastic-surgeon-seo-services" },
          { label: "Plastic Surgeon PPC", href: "/plastic-surgeon-ppc-services" },
          { label: "Plastic Surgeon Website Design & Development", href: "/plastic-surgeon-website-design-development-services" },
          { label: "Plastic Surgeon Social Media Marketing", href: "/plastic-surgeon-social-media-marketing-services" },
          { label: "Plastic Surgeon Reputation Management", href: "/plastic-surgeon-reputation-management-services" },
        ],
      },
      {
        label: "Legal",
        href: "/law-firm-digital-marketing",
        // subItems: [
          // { label: "Legal SEO", href: "#" },
          // { label: "Legal PPC", href: "#" },
        // ],
      },
    ],
  },
  {
    label: "Platform Expertise",
    href: "/platform-expertise",
    children: [
      {
        label: "CMS",
        href: "/cms-digital-marketing-agency",
        subItems: [
          { label: "Shopify", href: "/cms-shopify" },
          { label: "Wordpress", href: "/cms-wordpress" },
        ],
      },
      {
        label: "CRM Customisation Services",
        href: "/crm-customisation-digital-marketing-agency",
        subItems: [
          { label: "Zoho", href: "/crm-zoho" },
          { label: "Vtiger CRM", href: "/crm-vtiger" },
          { label: "Hubspot CRM", href: "/crm-hubspot" },
        ],
      },
      {
        label: "AI Services",
        href: "/artificial-digital-services-agency",
        subItems: [
          { label: "Claude", href: "claude" },
          { label: "Kilocode", href: "kilocode" },
          { label: "ChatGPT", href: "chatgpt" },
          { label: "Bolt", href: "bolt" },
        ],
      },
    ],
  },
  { label: "Contact us", href: "/contact-us" },
];

export default NAV_ITEMS;
