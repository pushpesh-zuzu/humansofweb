...
// ── Slide count per screen ───────────────────────────────────────────────────
function getSlideBasis() {
  if (typeof window === "undefined") return "100%";
  const w = window.innerWidth;
  if (w < 640) return "100%";
  if (w < 1024) return "50%";
  if (w < 1280) return "33.333%";
  return "25%";
}

// ── Main Component ───────────────────────────────────────────────────────────
export default function ServicesSlider() {
  const [slideBasis, setSlideBasis] = useState(null);
  ...
  useEffect(() => {
    const update = () => setSlideBasis(getSlideBasis());
    update();
    window.addEventListener("resize", update);
    return () => window.removeEventListener("resize", update);
  }, []);

  // Slide basis change hone par embla reinit karo
  useEffect(() => {
    if (emblaApi) emblaApi.reInit();
  }, [slideBasis, emblaApi]);

  if (!slideBasis) return null;

  return (
    <section className="w-full bg-white">
      <div className="mx-auto px-8">
        <div className="relative" style={{ paddingTop: 10, paddingBottom: 10 }}>
